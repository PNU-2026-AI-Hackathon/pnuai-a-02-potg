...
- Session 관계: 5개 ProgramCase에 Session row 20개가 존재한다. 실제 회차 수는 `COUNT(DISTINCT Session.id)`로 집계한다. Session row가 없는 나머지 344개는 관찰된 단회차가 아니라 파일럿 정책상 1회로 fallback한다.
...
Session 관계가 있는 사례가 5건뿐이라 회차 정보는 가장 큰 결측이다. `ONE_OFF` 344건은 실제 단회차 344건이라는 뜻이 아니라 Session 관계가 없을 때 1회로 처리한 파일럿 가정이다. Attachment 보완은 검색 가능한 정보를 늘리지만 편향 가능성이 있다. 30건은 UUID 하드코딩 없이 taxonomy·대상·회차·정보 의존성·UNKNOWN 포함 여부의 희소 범주를 우선 덮는 deterministic greedy 방식으로 선정한다. 제목만 명확한 사례, Session 의존 사례, Attachment 의존 사례를 모두 포함한다.
