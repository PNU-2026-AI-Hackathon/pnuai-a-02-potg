...
로그인 API를 먼저 세우고, 회원가입에 관심분야 기능을 추가하였다가 한 번 되돌린 뒤 DB 모델을 다시 잡았습니다. 이후 세션과 역할 권한을 넣고, 마지막에 화면과 입력 검증을 손봤습니다.
...
F09에서 만든 기획서를 입력으로 받아, 주민이 응답하는 공개 페이지와 사서가 결과를 보는 화면을 이어 붙였습니다.
