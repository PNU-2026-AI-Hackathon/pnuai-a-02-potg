...
const STUDIO_ANONYMOUS_VOTER_COOKIE_NAME = 'moira-studio-anonymous-voter';
...
export async function getBackendStudioVoteHeaders(): Promise<Record<string, string>> {
  const cookieStore = await cookies();
  const token = cookieStore.get(AUTH_COOKIE_NAME)?.value;
  if (token) return { Authorization: `Bearer ${token}` };

  let voterId = cookieStore.get(STUDIO_ANONYMOUS_VOTER_COOKIE_NAME)?.value;
  if (!voterId) {
    voterId = randomUUID();
    cookieStore.set(STUDIO_ANONYMOUS_VOTER_COOKIE_NAME, voterId, {
      httpOnly: true, sameSite: 'lax', path: '/', maxAge: STUDIO_ANONYMOUS_OWNER_COOKIE_MAX_AGE,
    });
  }
  return { 'X-Studio-Anonymous-Voter-Id': voterId };
}
