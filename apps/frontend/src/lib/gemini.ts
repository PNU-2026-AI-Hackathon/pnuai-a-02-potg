...
const fallbackModels = Array.from(new Set(
  [process.env.GEMINI_MODEL, ...allowedGeminiModels]
...
export function resolveModels(requested?: unknown) {
  const model = typeof requested === 'string' && allowedGeminiModels.includes(requested as GeminiModel)
    ...
    : null;
  return model
    ? [model, ...fallbackModels.filter((fallbackModel) => fallbackModel !== model)]
    : fallbackModels;
}
...
/** 모델 미지원·할당량 초과·일시 장애이면 다음 모델로 넘어간다. */
export function shouldTryNextGeminiModel(status: number, message: string) {
  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        ...
        if (shouldTryNextGeminiModel(response.status, lastError)) continue;
      ...
      try {
        ...
        lastError = 'Gemini 응답을 JSON으로 읽지 못했습니다.';
        continue;
      }
    } catch (error) {
      console.error(`Gemini request failed (${model}):`, error);
      lastError = `Gemini API에 연결하지 못했습니다. (${model})`;
    }
  }
  return { ok: false, error: lastError, status: 502 };
}
