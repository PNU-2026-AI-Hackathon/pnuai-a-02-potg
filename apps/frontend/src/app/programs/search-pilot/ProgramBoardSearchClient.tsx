...
type Profile = 'title' | 'title+intro' | 'title+intro+target';
type SearchResult = {
  ...
  detailLevel: 'detailed' | 'partial' | 'basic'; detailReason: string; sessionCount: number;
  sourceType: 'text' | 'attachment';
};
type SearchResponse = { query: string; limit: number; model: string; profile: Profile; candidateCount: number; eligibleCount: number; requestedAudience: string | null; reranking: string; results: SearchResult[] };
...
export default function ProgramBoardSearchClient() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const params = new URLSearchParams({ q: query.trim(), limit: String(limit), profile });
  ...
  async function createDraft() {
    ...
    try {
      const response = await fetch('/api/studio/generate', {
        ...
        body: JSON.stringify({ prompt: query.trim(), conditions: {}, agenda: null, referencesMarkdown: context, model: generationModel }),
      });
    ...
    <main className="programSearchPilotPage">
      <section className="programSearchPilotShell">
        ...
        <header className="programSearchHeader">
          ...
          <span>텍스트 정제 17건과 첨부 정제 20건, 총 37건을 대상으로 검색과 기획서 생성을 검증하는 화면입니다.</span>
        </header>
        <form className="programSearchForm" onSubmit={submit}>
          ...
          <div className="programSearchOptions">
            ...
            <label><span>검색 프로필</span><select value={profile} onChange={(event) => setProfile(event.target.value as Profile)}><option value="title">제목</option><option value="title+intro">제목 + 소개·목표</option><option value="title+intro+target">제목 + 소개·목표 + 대상</option></select></label>
        ...
        {data && <section className="programSearchResults">
          <div className="programSearchResultHeading"><div><h2>검색 결과</h2><p>{data.candidateCount}개 후보 중 최소 기준 통과 {data.eligibleCount}개 · {data.model} · {data.profile}{data.requestedAudience ? ` · 대상: ${data.requestedAudience}` : ''}</p></div><span>적합한 결과가 부족하면 선택한 Top N보다 적게 표시합니다.</span></div>
          <div>{data.results.map((result) => <article className="programSearchResultCard" key={result.sourceId}>
            ...
            <div className="programSearchResultBody"><div className="programSearchResultTitle"><h3><Link href={result.sourceType === 'attachment' ? `/programs/attachment-review/${result.sourceId}` : `/programs/${result.sourceId}`}>{result.title}</Link></h3><b>의미 유사도 {result.similarity.toFixed(3)} <small>({(result.similarity * 100).toFixed(1)}%)</small></b></div>
              ...
              <p className={`programSearchDetail is-${result.detailLevel}`}>상세도 {result.detailLevel} · {result.detailReason}{result.sessionCount ? ` · ${result.sessionCount}회차` : ''}</p>
