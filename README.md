<div align="center">
  ...
  <p>
    <font color="#6a737d">
      ...
      지역 주민은 원하는 프로그램을 제안하고, 사서는 MOIRA STUDIO로 주민 의견과 기존 사례를 참고해 더 쉽고 빠르게 기획안을 생성합니다. <br />
...
| 구분 | 기능 | 상세기능 | 설명 | 권한 |
| --- | --- | --- | --- | --- |
| 회원 | 회원가입 및 로그인 | 계정 유형 선택, 기본 정보 입력, JWT 기반 로그인 | 주민과 사서 계정을 구분하여 가입하고, 로그인 후 개인화 기능과 사서 전용 기능을 이용할 수 있습니다. | 공통 |
| 회원 | 관심분야 설정 | 관심분야 조회, 선택, 수정 | 사용자의 관심 주제를 저장하여 프로그램 탐색과 마이페이지 활동 관리에 활용합니다. | 로그인 사용자 |
| 홈 | 서비스 안내 | 서비스 흐름 안내, 주요 기능 진입, 추천 섹션 제공 | 주민 제안, AI 기획, 수요조사로 이어지는 서비스 흐름을 소개하고 주요 화면으로 이동할 수 있도록 합니다. | 공통 |
| 도서관 | 우리 동네 도서관 찾기 | 도서관 검색, 유형·위치 확인, 최근 프로그램 연결 | 지역 도서관 정보를 확인하고 해당 도서관과 관련된 프로그램을 함께 탐색할 수 있습니다. | 공통 |
| 프로그램 | 프로그램 둘러보기 | 목록 조회, 키워드 검색, 대상·도서관·모집상태 필터 | 정제된 도서관 프로그램 데이터를 조건별로 탐색하고 현재 모집 현황을 확인할 수 있습니다. | 공통 |
| 프로그램 | 프로그램 상세 및 신청 연결 | 상세 정보 확인, 원본 신청 페이지 이동 | 프로그램의 운영 정보와 모집 정보를 확인한 뒤 공공예약 서비스의 신청 페이지로 이동할 수 있습니다. | 공통 |
| 프로그램 | 프로그램 일정 | 월별 캘린더, 신청 기간 표시, 날짜별 일정 확인 | 여러 프로그램의 신청 기간을 달력 형태로 확인하여 원하는 시기의 프로그램을 쉽게 찾을 수 있습니다. | 공통 |
| 프로그램 | 관심 프로그램 | 관심 등록, 관심 해제, 마이페이지 조회 | 다시 보고 싶은 프로그램을 저장하고 마이페이지에서 모아 볼 수 있습니다. | 로그인 사용자 |
| 커뮤니티 | 우리동네 아이디어 게시판 | 아이디어 작성, 검색, 분류, 댓글, 공감 | 주민이 원하는 도서관 프로그램이나 지역 의제를 제안하고 다른 주민과 의견을 나눌 수 있습니다. | 공통 |
| 커뮤니티 | 도서관 소식 게시판 | 공지·행사 게시글 조회, 검색, 좋아요, 저장 | 도서관 공지와 행사·프로그램 소식을 확인하고 관심 있는 글을 저장할 수 있습니다. | 공통 |
| MOIRA STUDIO | 기획 시작 | 직접 입력 모드, 주민 아이디어 선택 모드, 조건 설정 | 사서가 직접 프로그램 아이디어를 입력하거나 주민 제안을 선택하고, 분야·대상·운영 기간 조건을 설정해 기획을 시작합니다. | 사서 |
| MOIRA STUDIO | AI 기획안 생성 | 참고 사례 검색, Gemini 기반 초안 생성, 단계별 생성 상태 표시 | 입력한 아이디어와 조건, 기존 프로그램 사례를 바탕으로 AI가 도서관 프로그램 기획안 초안을 생성합니다. | 사서 |
| MOIRA STUDIO | 기획안 편집 | 항목별 직접 수정, AI로 다듬기, 수정 결과 반영 | 생성된 기획안을 사서가 직접 수정하거나 필요한 항목만 선택해 AI의 도움으로 보완할 수 있습니다. | 사서 |
| MOIRA STUDIO | 기획서 관리 | 기획서 저장, 조회, 삭제, 단계 변경 | 작성한 기획서를 저장하고 `기획 중` → `수요조사 중` → `수요조사 완료` → `기획서 확정` 단계로 관리합니다. | 사서 |
| 수요조사 | 수요조사 참여 | 참여 의향 선택, 선호 시간대 응답, 응답 수정·취소 | 주민이 기획 중인 프로그램에 대한 참여 의향과 선호 시간대를 제출할 수 있습니다. | 공통 |
| 수요조사 | 수요조사 결과 확인 | 응답 수 집계, 참여 의향 분석, 선호 시간대 분석 | 사서가 주민 응답 결과를 확인하여 실제 프로그램 개설 여부와 운영 일정을 결정하는 데 참고할 수 있습니다. | 사서 |
| 마이페이지 | 내 활동 관리 | 프로필 수정, 작성글·댓글·관심글·좋아요·관심 프로그램 조회 | 사용자의 계정 정보와 서비스 활동 내역을 한곳에서 확인하고 관리할 수 있습니다. | 로그인 사용자 |
...
|   `-- backend/      # Express/Prisma API 서버와 데이터 배치
